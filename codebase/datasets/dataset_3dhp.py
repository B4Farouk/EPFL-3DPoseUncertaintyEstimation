...
from datasets.chunked_generator import ChunkedGenerator_3DHP as ChunkedGenerator
from datasets.utils             import  normalize_screen_coordinates, create_graph, deterministic_random


class Fusion(data.Dataset):
    def __init__(self, opt, batchSize, root_path, train=True, MAE=False, use_graphs=False, dict_vals_graph={}):# 
        ...
        self.dict_vals_graph = dict_vals_graph
        ...
        if self.train:
            ...
            #                                                                        subset=self.subset)
            self.generator = ChunkedGenerator(batchSize // opt.stride, None, self.poses_train, self.poses_train_2d, None, chunk_length=self.stride, pad=self.pad,
                                              augment=opt.data_augmentation, reverse_aug=opt.reverse_augmentation, kps_left=self.kps_left, kps_right=self.kps_right,
                                              joints_left=self.joints_left, joints_right=self.joints_right, out_all=opt.out_all, MAE=MAE, train = True)
            ...
            #                                                                     subset=self.subset)
            self.generator = ChunkedGenerator(batchSize // opt.stride, None, self.poses_test, self.poses_test_2d, self.valid_frame, out_all=opt.out_all_test,
                                              pad=self.pad, augment=False, kps_left=self.kps_left, kps_right=self.kps_right, joints_left=self.joints_left,
    ...
    def __getitem__(self, index):
        ...
        if self.MAE:
            cam, input_2D, seq, subject, cam_ind = self.generator.get_batch(seq_name, start_3d, end_3d, flip, reverse)
        ...
        if not self.use_graphs:
            ...
            graph = create_graph(**self.dict_vals_graph)

        if self.MAE:
            if self.train == True: # cam, input_2D_update, seq, subject, scale, bb_box, cam_ind
                ret_vals = {'cam' : cam, 'input_2D_update' : input_2D_update, 'seq' : seq, 'subject' : subject, 'scale' : scale, 'bb_box' : bb_box, 'cam_ind' : cam_ind} #, 'graph' : graph}
            else: # cam, input_2D_update, seq, scale, bb_box
                ret_vals = {'cam' : cam, 'input_2D_update' : input_2D_update, 'seq' : seq, 'scale' : scale, 'bb_box' : bb_box} #, 'graph' : graph}
        else:
            if self.train == True: # cam, gt_3D, input_2D_update, seq, subject, scale, bb_box, cam_ind
                ret_vals = {'cam' : cam, 'gt_3D' : gt_3D, 'input_2D_update' : input_2D_update, 'seq' : seq, 'subject' : subject, 'scale' : scale, 'bb_box' : bb_box, 'cam_ind' : cam_ind} #, 'graph' : graph}
            else:# cam, gt_3D, input_2D_update, seq, scale, bb_box
                ret_vals = {'cam' : cam, 'gt_3D' : gt_3D, 'input_2D_update' : input_2D_update, 'seq' : seq, 'scale' : scale, 'bb_box' : bb_box} #, 'graph' : graph}

        return ret_vals, graph