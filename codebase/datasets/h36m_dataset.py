...
from datasets.mocap_dataset import MocapDataset
from datasets.skeleton      import Skeleton

from datasets.utils             import deterministic_random, world_to_camera, normalize_screen_coordinates, create_graph
from datasets.chunked_generator import ChunkedGenerator


h36m_skeleton = Skeleton(parents=[-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 12, 16, 17, 18, 19, 20, 19, 22, 12, 24, 25, 26, 27, 28, 27, 30],
    ...
    def __init__(self, path, opt, remove_static_joints=True):
        ...
        for subject, actions in data.items():
            ...
            for action_name, positions in actions.items():
                self._data[subject][action_name] = {'positions': positions, 'cameras': self._cameras[subject]}
...
class Fusion(data.Dataset):
    def __init__(self, batchSize, opt, dataset, root_path, train=True, MAE=False, tds=1, use_graphs=False, dict_vals_graph={}): #
        ...
        if self.train:
            ...
            self.cameras_train, self.poses_train, self.poses_train_2d = self.fetch(dataset, self.train_list,
                                                                                   subset=self.subset)
            self.generator = ChunkedGenerator(batchSize // opt.stride, self.cameras_train, self.poses_train, self.poses_train_2d, self.stride, pad=self.pad,
                                              augment=opt.data_augmentation, reverse_aug=opt.reverse_augmentation, kps_left=self.kps_left, kps_right=self.kps_right,
                                              joints_left=self.joints_left, joints_right=self.joints_right, out_all=opt.out_all, MAE=MAE, tds=tds)
            ...
            self.cameras_test, self.poses_test, self.poses_test_2d = self.fetch(dataset, self.test_list,
                                                                                subset=self.subset)
            self.generator = ChunkedGenerator(batchSize // opt.stride, self.cameras_test, self.poses_test, self.poses_test_2d, out_all=opt.out_all_test,
    ...
    def __getitem__(self, index):
        ...
        if self.MAE:
            cam, input_2D, action, subject, cam_ind = self.generator.get_batch(seq_name, start_3d, end_3d, flip, reverse)
        ...
        if self.MAE: # return cam, input_2D_update, action, subject, scale, bb_box, cam_ind
            ret_vals = {'cam' : cam, 'input_2D_update' : input_2D_update, 'action' : action, 'subject' : subject, 'scale' : scale, 'bb_box' : bb_box, 'cam_ind' : cam_ind} #, 'graph' : graph}
        else: # return cam, gt_3D, input_2D_update, action, subject, scale, bb_box, cam_ind
            ret_vals = {'cam' : cam, 'gt_3D' : gt_3D, 'input_2D_update' : input_2D_update, 'action' : action, 'subject' : subject, 'scale' : scale, 'bb_box' : bb_box, 'cam_ind' : cam_ind} #, 'graph' : graph}
