...
def get_opt_cpn(config):
    args_train = {'dataset'               : config.dataset_name.lower(),
                  'keypoints'             : 'cpn_ft_h36m_dbb' if config.dataset_name.lower() else None, # TODO - FOR OTHER DATASETS.
                  'crop_uv'               : 0,
                  'root_path'             : '/cvlabdata2/home/soumava/codes/Step1/GCN/code/Modulated_GCN/cpn_keypoints/',
                  'downsample'            : 1,
                  'subset'                : 1,
                  'stride'                : config.every_nth_frame_train_annotated,
                  'data_augmentation'     : config.data_augmentation_cpn,
                  'test_augmentation'     : config.test_augmentation_cpn,
                  'actions'               : '*',
                  'reverse_augmentation'  : config.reverse_augmentation_cpn,
                  ...
                  'pad'                   : 0,
                  'subjects_train'        : 'S1,S5,S6,S7,S8', # TODO - FOR OTHER DATASETS.
                  'subjects_test'         : 'S9,S11',         # TODO - FOR OTHER DATASETS.
                  'n_joints'              : 17,
                  'out_joints'            : 17,
                  'in_channels'           : 2,
                  'out_channels'          : 3,
                  'device'                : device,
                  'module_gcn_reload'     : 1,
                  'post_refine_reload'    : 1,
                  'previous_dir'          : '/cvlabdata2/home/soumava/codes/Step1/GCN/code/Modulated_GCN/pretrained-models',
                  'load_pretrained_model' : config.load_pretrained_weights,
                  'module_gcn_model'      : 'model_module_gcn_7_eva_post_4939.pth',  # TODO - FOR OTHER DATASETS.
                  'post_refine_model'     : 'model_post_refine_7_eva_post_4939.pth', # TODO - FOR OTHER DATASETS.
                  'out_all'               : True
                  }

    args_test  = {'dataset'               : config.dataset_name.lower(),
                  'keypoints'             : 'cpn_ft_h36m_dbb' if config.dataset_name.lower() else None,  # TODO - FOR OTHER DATASETS.
                  'crop_uv'               : 0,
                  'root_path'             : '/cvlabdata2/home/soumava/codes/Step1/GCN/code/Modulated_GCN/cpn_keypoints/',
                  'downsample'            : 1,
                  'subset'                : 1,
                  'stride'                : max(config.every_nth_frame_validation, config.every_nth_frame_test),
                  'data_augmentation'     : config.data_augmentation_cpn,
                  'test_augmentation'     : config.test_augmentation_cpn,
                  'actions'               : '*',
                  'reverse_augmentation'  : config.reverse_augmentation_cpn,
    ...
    opt_cpn_train = namedtuple('Struct', args_train.keys())(*args_train.values())
    opt_cpn_test  = namedtuple('Struct', args_test.keys())(*args_test.values())
    return opt_cpn_train, opt_cpn_test
...
def get_cpn_dataset_train(opt):
    root_path = opt.root_path
    if opt.dataset == 'h36m':
        dataset_path = os.path.join(root_path, 'data_3d_' + opt.dataset + '.npz')
        print("Obtaining the CPN data from {} for the dataset {}.".format(dataset_path, opt.dataset))
        h36m_skeleton_train = copy.copy(h36m_skeleton)
        dataset             = Human36mDataset(dataset_path, opt, skeleton=h36m_skeleton_train)
    else:
        raise KeyError('Invalid dataset')
    return dataset


def get_cpn_dataset_test(opt):
    root_path = opt.root_path
    if opt.dataset == 'h36m':
        dataset_path = os.path.join(root_path, 'data_3d_' + opt.dataset + '.npz')
        print("Obtaining the CPN data from {} for the dataset {}.".format(dataset_path, opt.dataset))
        h36m_skeleton_test = copy.copy(h36m_skeleton)
        dataset            = Human36mDataset(dataset_path, opt, skeleton=h36m_skeleton_test)
    else:
        raise KeyError('Invalid dataset')
    return dataset
...
h36m_cameras_extrinsic_params = {'S1': [
                                        {
...
class Human36mDataset(MocapDataset):
    def __init__(self, path, opt, skeleton, remove_static_joints=True):
